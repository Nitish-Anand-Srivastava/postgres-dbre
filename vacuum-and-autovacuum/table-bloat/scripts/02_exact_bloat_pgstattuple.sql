...
PURPOSE:
Exact physical bloat scan for one specific table using the pgstattuple extension.
...
EXPECTED IMPACT:
Performs a full table scan (or sampled scan for pgstattuple_approx) and takes a light read lock; can be I/O-intensive on very large tables.
...
EXECUTION ORDER:
Step 02 of workflow 'vacuum-and-autovacuum/table-bloat'
