# Scripts: Connection Pooling (PgBouncer) Considerations

Execution order, safety classification, and expected runtime for every script
in `connections/connection-pooling/scripts/`.

| Order | Script | Purpose | Safety | Expected Runtime |
| ----- | ------ | ------- | ------ | ---------------- |
| 01 | `01_pooler_connection_footprint.sql` | Checks how many database-side connections the pooler's application_name/user is actually consuming, to distinguish a database-side vs. pooler-side bottleneck. | READ ONLY | Low (sub-second to a few seconds) |
