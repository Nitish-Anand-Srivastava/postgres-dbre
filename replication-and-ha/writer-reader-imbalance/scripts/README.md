# Scripts: Writer/Reader Load Imbalance

Execution order, safety classification, and expected runtime for every script
in `replication-and-ha/writer-reader-imbalance/scripts/`.

| Order | Script | Purpose | Safety | Expected Runtime |
| ----- | ------ | ------- | ------ | ---------------- |
| 01 | `01_writer_query_type_breakdown.sql` | Breaks down query activity on the writer by application to identify read-heavy traffic that could potentially be offloaded to readers. | READ ONLY | Low (sub-second to a few seconds) |
